...
public class NativesGenerator extends JNIGenerator {
	...
	private void generateConstantsInitializer(JNIMethod method) {
		...
		outputln("JNIEXPORT void JNICALL " + clazz.getSimpleName() + "_NATIVE(" + toC(method.getName())
		...
		for (JNIField field : constants) {
			...
			String fieldId = "(*env)->GetStaticFieldID(env, that, \"" + field.getName() + "\", \""
					+ type.getTypeSignature(allowConversion) + "\")";
			if (isCPP) {
				fieldId = "env->GetStaticFieldID(that, \"" + field.getName() + "\", \""
						+ type.getTypeSignature(allowConversion) + "\")";
			}
			...
			if (type.isPrimitive()) {
				if (isCPP) {
					...
					output("\t(*env)->SetStatic" + type.getTypeSignature1(allowConversion) + "Field(env, that, "
							+ fieldId + ", ");
				}
	...
	public void generate(JNIMethod method) {
		...
		if (!(returnType.isType("void") || returnType.isPrimitive() || isSystemClass(returnType)
				|| returnType.isType("java.lang.String"))) {
		...
		if (paramType.isArray()) {
			...
			if (componentType.isPrimitive()) {
				...
				if ("long".equals(componentType.getName()) && param.isPointer()) {
					// This case is special as we may need to do pointer
					// conversions..
					// if your on a 32 bit system but are keeping track of the
					// pointers in a 64 bit long
			...
			} else if (param.getTypeClass().getFlag(ClassFlag.STRUCT)) {
				output("if (arg");
				output(iStr);
				outputln(") { ");
				outputln("\t\tfor (int i = 0;i< jobjectArraySize" + iStr + ";i++) {");
				outputln("\t\t\tjobject tsarg = (*env)->GetObjectArrayElement(env,arg" + iStr + ",i);");
				outputln("\t\t\tif(tsarg == NULL) goto fail;");
				output("\t\t\t");
				output("get");
				output(paramType.getSimpleName());
				output("Fields(env, tsarg");
				outputln(", &lparg" + iStr + "[i]);");
				outputln("\t\t}");
				outputln("\t}");
			} else {
				throw new Error("not done");
	...
	boolean generateLocalVars(JNIMethod method, List<JNIParameter> params, JNIType returnType, JNIType returnType64) {
		...
		for (int i = 0; i < params.size(); i++) {
			...
			if (paramType.isPrimitive() || isSystemClass(paramType))
				continue;
			output("\t");
			JNIClass d1 = param.getTypeClass();
			boolean dfs = d1.getFlag(ClassFlag.STRUCT);
			String jndiTypeName = paramType.getNativeName();
			if (paramType.isArray()) {
				JNIType componentType = paramType.getComponentType();
				if ("long".equals(componentType.getName()) && param.isPointer()) {
					...
					if (dfs && !d1.getFlag(ClassFlag.TYPEDEF)) {
						output("struct ");
					}
					output(jndiTypeName);
					output(" *lparg" + i + ";");
					outputln();
					output("\t");
					output("int jobjectArraySize" + i + " = (*env)->GetArrayLength(env,arg" + i + ");");
					outputln();
					output("\t");
					output("lparg" + i + " = (");
					if (dfs && !d1.getFlag(ClassFlag.TYPEDEF)) {
					...
					output(jndiTypeName + " *) malloc(sizeof(struct " + jndiTypeName + ") * jobjectArraySize" + i
							+ ");");
				}
	...
	void generateDynamicFunctionCall(JNIMethod method, List<JNIParameter> params, JNIType returnType,
			JNIType returnType64, boolean needsReturn) {
		outputln("/*");
	...
	static String[] getNativeNameParts(JNIMethod method) {
		...
		if (className == null) {
			throw new Error(String.format("Could not determine object type name of method '%s'",
					method.getDeclaringClass().getSimpleName() + "." + method.getName()));
		}
		return new String[] { className, methodName };
	}

	void generateFunctionCall(JNIMethod method, List<JNIParameter> params, JNIType returnType, JNIType returnType64,
			boolean needsReturn) {
		String name = method.getName();
		...
		if (objc_struct) {
			...
			output(" **)arg1)[arg0])");
			paramStart = 1;
		} else if (method.getFlag(MethodFlag.CPP_METHOD) || method.getFlag(MethodFlag.SETTER)
				|| method.getFlag(MethodFlag.GETTER) || method.getFlag(MethodFlag.ADDER)) {
			...
			output("(");
			if (params.isEmpty()) {
				throw new Error(String.format("C++ bound method '%s' missing the 'this' parameter",
						method.getDeclaringClass().getSimpleName() + "." + method.getName()));
			}
			JNIParameter param = params.get(0);
			if (param.getFlag(ArgFlag.BY_VALUE))
					...
					if (cast.length() != 0 && !cast.equals("()")) {
						...
						if (!(paramType.isPrimitive() || paramType.isArray())) {
							if (param.getTypeClass().getFlag(ClassFlag.STRUCT)
									&& !param.getTypeClass().getFlag(ClassFlag.TYPEDEF)) {
								output("struct ");
							}
						}
						output(paramType.getTypeSignature4(!paramType.equals(paramType64),
								param.getFlag(ArgFlag.BY_VALUE)));
					}
				}
				output("))");
			}
		...
		if ((method.getFlag(MethodFlag.SETTER) && params.size() == 3)
				|| (method.getFlag(MethodFlag.GETTER) && params.size() == 2)) {
			output("[arg1]");
			paramStart++;
		}
	...
	void generateFunctionBody(JNIMethod method, String function, String function64, List<JNIParameter> params,
			JNIType returnType, JNIType returnType64) {
		outputln("{");
		...
		if (name.startsWith("_"))
			name = name.substring(1);
		boolean isMemove = (name.equals("memmove") || name.equals("MoveMemory")) && params.size() == 2
				&& returnType.isType("void");
		if (isMemove) {
	...
	void generateFunctionPrototype(JNIMethod method, String function, List<JNIParameter> params, JNIType returnType,
			JNIType returnType64, boolean singleLine) {
		output("JNIEXPORT ");
		output(returnType.getTypeSignature2(!returnType.equals(returnType64)));
		output(" JNICALL ");
		output(method.getDeclaringClass().getSimpleName());
